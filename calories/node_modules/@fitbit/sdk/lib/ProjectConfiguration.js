...
function getAllPermissionTypes(enableProposedAPI) {
    return [
...
function validateRequestedPermissions({ enableProposedAPI, requestedPermissions, }) {
    return constrainedSetDiagnostics({
        ...
        knownValues: getAllPermissionTypes(!!enableProposedAPI).map((permission) => permission.key),
...
function validateDefaultLanguage(config) {
    ...
    if (!languageTag_1.validateLanguageTag(config.defaultLanguage)) {
        diagnostics.pushFatalError(`Default language is an invalid language tag: ${config.defaultLanguage}. Must be ${humanize_list_1.default(languageTag_1.supportedTags, { conjunction: 'or' })}.`);
    }
    return diagnostics;
}
...
function validateStorageGroup(config) {
    const diagnostics = new DiagnosticList_1.default();
    const hasRequestedPermission = getAllPermissionTypes(!!config.enableProposedAPI)
    ...
    if (hasRequestedPermission) {
        ...
        else if (!validator_1.isUUID(String(config.developerID))) {
